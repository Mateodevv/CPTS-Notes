...
```shell-session
Tachipy@htb[/htb]$ john --format=<hash_type> <hash or hash_file>
```
...
```shell-session
Tachipy@htb[/htb]$ john --format=sha256 hashes_to_crack.txt
```
...
```shell-session
Tachipy@htb[/htb]$ john --wordlist=<wordlist_file> --rules <hash_file>
```
...
```shell-session
Tachipy@htb[/htb]$ john --incremental <hash_file>
```
